                ...
                <div class="mb-4">
                    ...
                    <form action="{{route('account-oder')}}" method="GET">
                        <div class="search-form-2 search-form-mobile">
                            ...
                            <input type="text"  name="search" value="{{ request('search') }}" class="form-control text-title" placeholder="{{ translate('search_by_order_ID') }}" autocomplete="off">
                            <button type="submit" class="clear border-0">
                                @if (request('search') != null)
                                    <a href="{{route('account-oder',['show_order'=>request('show_order')])}}">{{translate('clear')}}</a>
                                @else
                                    ...
                                    <tr class="bg-section">
                                            ...
                                            <div class="cart-product ">
                                                <label class="form-check thisIsALinkElement" data-linkpath="{{ route('account-order-details', ['id'=>$order->id]) }}">
                                                    @if($order->seller_is == 'seller')
                                                        <img loading="lazy" alt="{{ translate('shop') }}" src="{{ getStorageImages(path: $order?->seller?->shop->image_full_url , type:'shop') }}">
                                                    @elseif($order->seller_is == 'admin')
                                                        <img loading="lazy" alt="{{ translate('logo') }}" src="{{ getStorageImages(path: $web_config['fav_icon'], type:'logo') }}">
                                                    @endif
                                                </label>
                                                <div class="cont">
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-3 align-items-center">
                                                <a href="{{ route('account-order-details', ['id'=>$order->id]) }}" class="btn-circle" data-bs-toggle="tooltip" title="{{translate('view_details')}}">
                                                    <i class="bi bi-eye-fill"></i>
                                                </a>
                                                <a href="{{route('generate-invoice',[$order->id])}}" class="btn-circle border-0" data-bs-toggle="tooltip" title="{{translate('download_invoice')}}">
                                    ...
                                    <div class="cont">
                                        ...
                                        <div class="text-basa mb-1">
                                            <span>{{translate('total_price')}} :
                                                <strong>{{\App\Utils\currency_converter($order['order_amount'])}}</strong>
                                            </span>
                                        </div>
                                        <div class="text-basa mb-1">
                                            <span>{{translate('qty')}} :
                ...
                @else
                    <div class="text-center pt-5 w-100">
                        <div class="text-center mb-5">
                            <img loading="lazy" src="{{ theme_asset('assets/img/icons/order.svg') }}" alt="{{ translate('order') }}">
