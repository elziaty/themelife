                ...
                <div class="__chat-area">
                        ...
                        <ul class="nav nav-tabs nav--tabs-3">
                            <li class="nav-item">
                                <a href="{{route('chat', ['type' => 'vendor'])}}"
                                   class="nav-link {{Request::is('chat/vendor')?'active':''}}">{{ translate('vendor') }}</a>
                            </li>
                            <li class="nav-item">
                                    ...
                                    @if(isset($allChattingUsers) && count($allChattingUsers) > 0)
                                        @foreach($allChattingUsers as $key => $chatting)
                                            ...
                                            <div class="nav-item w-100">
                                                <a href="javascript:" class="chat_list nav-link get-ajax-message-view {{ $loop->iteration == 1 ? 'active' : ''}}"  data-user-id="{{ $userId }}">
                                                    @if($chatting->delivery_man_id)
                                                        <img alt="{{ translate('user') }}" class="img"
                                                    ...
                                                    <div class="content">
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center msg mt-2">
                                                            @if($chatting->message)
                    ...
                    <div class="__chat-content">
                            ...
                            <div class="tab-pane fade show active position-relative">
                                <div class="__chat-content-header">
                                    <a href="javascript:" class="chat-author">
                                        @if($userType == 'admin')
                                            <img alt="" class="img profile-image" loading="lazy" src="{{ getStorageImages(path: $web_config['fav_icon'], type: 'shop') }}">
                                        @elseif($userType == 'vendor')
                                            <img alt="" class="img profile-image" loading="lazy" src="{{ getStorageImages(path: $lastChatUser->image_full_url, type: 'shop') }}">
                                        @elseif($userType == 'delivery-man')
                                            <img alt="" class="img profile-image" loading="lazy" src="{{ getStorageImages(path: $lastChatUser->image_full_url, type: 'avatar') }}">
                                        @endif

                                        <div class="content">
                                            <h5 id="name" class="name mt-11 profile-name">
                                                {{ $userType == 'admin' ? getWebConfig('company_name') : ($userType == 'vendor' ? $lastChatUser->name : ($lastChatUser?->f_name.' '.$lastChatUser?->l_name))  }}
                                            </h5>
                                        </div>
                                    </a>
                                </div>
                                <div class="__chat-content-body scroll_msg" >
                                    <ul class="__chat-content-body-messages msg_history" id="chatting-messages-section">
                        ...
                        <div class="__chat-content-footer">
                            <form method="post" class="chatting-messages-form" enctype="multipart/form-data">
                                @csrf
                                <div id="view" class="view-img"></div>
                                <div class="d-flex align-items-start">
                                    <div class="d-flex align-items-center m-0 gap-2 align-self-start">
                                    ...
                                    <div class="position-relative d-flex w-0 flex-grow-1">
                                        <div class="w-0 flex-grow-1">
                                                <textarea class="form-control border-0 resize-none ps-4 py-0" name="message" id="msgInputValue"
                                        ...
                                        <button type="submit" class="btn ms-1" id="message-send-button">
