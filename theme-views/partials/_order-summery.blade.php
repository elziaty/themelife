...
@php($get_shipping_cost_saved_for_free_delivery=\App\Utils\CartManager::get_shipping_cost_saved_for_free_delivery(type: 'checked'))
@php($coupon_dis=0)
@if($cart->count() > 0)
...
<div class="total-cost-wrapper">
    <div class="total-cost-area text-capitalize">
        <h5 class="mb-4">{{translate('order_summary')}} <small
                    class="text-base font-regular text-small">({{count(\App\Utils\CartManager::get_cart(type: 'checked'))}} {{translate('items')}}
                )</small></h5>

        <div class="overflow-y-auto h--28rem">
            @if(auth('customer')->check())
                @php($cart_list = \App\Models\Cart::where(['customer_id' => auth('customer')->id(), 'is_guest' => 0,'is_checked'=>1])->get()->groupBy('cart_group_id'))
            @elseif(getWebConfig(name: 'guest_checkout') && session()->has('guest_id') && session('guest_id'))
                @php($cart_list = \App\Models\Cart::where(['customer_id' => session('guest_id'), 'is_guest' => 1,'is_checked'=>1])->get()->groupBy('cart_group_id'))
            @endif
