...
function stock_check() {
    ...
    let minimum_order_quantity_msg = $(".minimum_order_quantity_msg").data(
        "text"
    );
...
function addToCart(formElement, redirect_to_checkout = "false", url = null) {
    if (
        checkAddToCartValidity(formElement) &&
        formElement.find("input[name=quantity]").val() != 0
    ) {
        $.ajaxSetup({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        ...
        let redirectToCheckout = redirect_to_checkout.toString();

        let existCartItem = $('.in_cart_key[name="key"]').val();
        let formActionUrl = formElement.attr("action");
        if (existCartItem !== "" && redirectToCheckout === "false") {
        ...
        $.post({
            ...
            data: formElement.serializeArray()
                .concat({
                    name: "buy_now",
                    value: redirectToCheckout === "true" ? 1 : 0,
                }),
            beforeSend: function () {
            },
            success: function (response) {
                ...
                if (response.status == 1) {
                    ...
                    actionAddToCartBtn
                        .children("span")
                        .html(actionAddToCartBtn.data("update-text"));
                    addToCartFunctionBtn
                        .children("span")
                        .html(addToCartFunctionBtn.data("update-text"));
...
function getUpdateProductAddUpdateCartBtn(response) {
    try {
        ...
        if (!productVariantExist) {
            ...
            actionAddToCartBtn
                .children("span")
                .html(actionAddToCartBtn.data("add-text"));
            addToCartFunctionBtn
                .children("span")
                .html(addToCartFunctionBtn.data("add-text"));
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (
                    data.quantity != 0 &&
                    data.quantity >
                        $("#add-to-cart-form .product_qty").attr("max")
                ) {
                    $("#add-to-cart-form .product_qty").attr(
                        "max",
                        data.quantity
                    );
                } else {
                    if (data.quantity <= 0) {
                        $("#add-to-cart-form .product_qty").val(
                            parseInt(
                                $("#add-to-cart-form .product_qty").attr("min")
                            )
                        );
                        $("#add-to-cart-form .product_qty").attr(
                            "max",
...
function resetAllInProductList() {
    $(".fashion_products_list_form_reset").on("click", function () {
        $(".filter_by_product_list_web").val(["default"]).trigger("change");
        $(".filter_select_input_div .select2-selection__rendered").text(
            $(".filter_select_input").data("primary_select")
        );
